  ...
  <el-container>
    ...
    <el-main>
      <el-table
        ...
        <el-table-column fixed="right" width="187">
          <template slot-scope="scope">
            <el-button type="primary" @click="goReport(scope.row)">
              获取报表
            </el-button>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    goReport(data) {
      this.$router.push({path: "/chains/report", query: {chainCode: data.chainCode, chainBatchCode: data.chainBatchCode}})
    }
  },
  mounted() {
    this.getData()
